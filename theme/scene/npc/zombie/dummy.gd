...
func _ready() -> void:
	...
	$sped.start()
	if enemy_id in InventoryManager.enemies_killed:
		queue_free()
	if InventoryManager.is_enemy_dead(enemy_id):
		queue_free()
...
func take_dmg(dmgad):
	...
	if health > 0:
		...
		if enemy_id != "":
			InventoryManager.mark_enemy_killed(enemy_id)
		is_dead = true
		if enemy_id != "" and enemy_id not in InventoryManager.enemies_killed:
			InventoryManager.enemies_killed.append(enemy_id)
		var head_off = false
